...
if MainMemberName == ""
  msg = LiteApp6.Utility.i18n("MainMemberName must be non-empty.");
...
function msg = errmsg(name)
  % !todo: Add the "{1}" placeholder support in LiteApp6.Utility.i18n and use it.
...
if class(target_string) == "string" && startsWith(target_string, "simscape.Value")
  ...
  try
    value_result = LiteApp6.Utility.getNumericValueFromString(value_string);
  catch exception
  ...
  if isnan(value_numeric)
    ...
    if string(class(eval_result)) == "simscape.Value"
      ...
      if unit_string == ""
        ...
        msg = LiteApp6.Utility.i18n("Physical unit is required to create a simscape.Value object for a number: ") + eval_result;
