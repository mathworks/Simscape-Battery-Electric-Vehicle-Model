function PropertyDictionary = buildPropertyDictionaryFromSimscapeBlock(BlockPath)
...
errorID = "buildPropertyDictionaryFromSimscapeBlock:";

if BlockPath == "" || not(contains(BlockPath, "/"))
  id = errorID + "InvalidBlockPath";
  msg = "Specified block is invalid.";
...
if block_type ~= "SimscapeBlock" && block_type ~= "SimscapeComponentBlock"
  id = errorID + "NotSimscapeBlock";
  msg = "Specified block is not a Simscape block: " + BlockPath;
...
if isempty(block_mask)
  id = errorID + "BlockHasNoMask";
  msg = "Specified block has no mask: " + BlockPath;
...
if all(size(mask_params) == [0 0])
  ...
  msg = "Specified block has no parameters.";
