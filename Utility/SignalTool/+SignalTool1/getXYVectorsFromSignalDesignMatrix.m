...
assert( issorted(x_points(:,1), "strictascend"), ...
  ...
  LiteApp6.Utility.i18n("X start vector must be strictly asending."))
...
if any(logical_index)
  ...
  assert( issorted(tmp_vec, "strictascend"), ...
    ...
    LiteApp6.Utility.i18n("X end vector must be strictly asending."))
  ...
  assert( isempty(logical_index), ...
    error_id + "InvalidX", ...
    LiteApp6.Utility.i18n("X start is after X end, which is invalid, at these rows: ") + num2str(logical_index'))
...
assert( all(not(isnan(y_points))), ...
  error_id + "invalidY", ...
  LiteApp6.Utility.i18n("Y data cannot have NaN."))
...
assert( all(dx > 0), ...
  ...
  LiteApp6.Utility.i18n("X data points are not strictly ascending."))
