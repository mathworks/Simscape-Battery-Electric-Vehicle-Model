classdef LiteAppLayout < handle
  ...
  methods
    ...
    function newGrid = NewArea(layout, NameValuePair)
      ...
      arguments (Input)
        layout (1,1)
        NameValuePair.Height (1,1) {CodeTool1.mustBeStringOrPositiveInteger} = "fit"
      end
      arguments (Output)
    ...
    function newGrid = NewColumn(layout, currentGrid, NameValuePair)
      ...
      arguments (Input)
        ...
        % such as "1x", "2x", "fit", 100, 200 etc.
        NameValuePair.Width (1,1) {CodeTool1.mustBeStringOrPositiveInteger} = "1x"
      end
      arguments (Output)
        newGrid (1,1) matlab.ui.container.GridLayout
      end
    ...
    function newGrid = NewSlot(layout, currentGrid, NameValuePair)
      ...
      arguments (Input)
        ...
        % You can pass values as long as it is supported by ColumnWidth,
        % such as "1x", "2x", "fit", 100, 200 etc.
        NameValuePair.Width (1,:) {CodeTool1.mustBeStringOrPositiveInteger} = "1x"
      end
      arguments (Output)
        newGrid (1,1) matlab.ui.container.GridLayout
      end
