function App = bevutil1_LookupTable1DBlockPlotApp(NameValuePair)
...
if isfield(NameValuePair, "ModelFilePath")
  if not(isfile(NameValuePair.ModelFilePath))
    ...
    msg = bevutil1.CodeUtil.i18n("The file specified for ModelFilePath is not valid.");
...
app_window.Name = bevutil1.CodeUtil.i18n("Lookup-Table 1D Block Plot App");

app_vertical_container = app_window.MainVerticalContainer;

column_grid = addVerticalGridLayout(app_vertical_container);
block_selector_ui = bevutil1.AppUtil.Component.BlockSelectorUI(column_grid);
block_selector_ui.GetOnly = true;
block_selector_ui.AutoGet = true;
block_selector_ui.FindBlockCallback = @bevutil1.ModelUtil.findLookupTable1DBlocks;
...
column_grid = addVerticalGridLayout(app_vertical_container);
open_fig_win_ui = bevutil1.AppUtil.Component.Hyperlink(column_grid);
open_fig_win_ui.Text = bevutil1.CodeUtil.i18n("Open in figure window");
open_fig_win_ui.HorizontalAlignment = "right";
open_fig_win_ui.HyperlinkClickedCallback = @() react_figwin();

column_grid = addVerticalGridLayout(app_vertical_container);
panel_ui = bevutil1.AppUtil.Graphics.Panel(column_grid);
panel_ui.ComponentHeight = 380;
  ...
  function react_figwin()
    ...
    bevutil1.ModelUtil.plotLookupTable1DBlocks(gcs, Blocks=block_name, ParentType="Axes", ParentAxes=axes(figure))
  end  % nested function
