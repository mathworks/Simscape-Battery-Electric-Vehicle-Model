function App = bev1mus_FileListApp(FileList, NameValuePair)
...
session = bev1mus.SearchUtil.searchText( ...
  "movegui", ...
...
bev1mus_FileListApp(session.Result, TopFolder=session.Searcher.States.TargetFolder)
%}
...
if class(FileList) == "string"
  if isempty(FileList) || (isscalar(FileList) && FileList == "")
    ...
    msg = bev1mus.CodeUtil.i18n("FileList string must be non-empty.");
  ...
  if isempty(FileList)
    ...
    msg = bev1mus.CodeUtil.i18n("FileList table must be non-empty.");
  ...
  if not(ismember("FilePath", column_names))
    ...
    msg = bev1mus.CodeUtil.i18n("Table must have FilePath column.");
...
app_window.Name = bev1mus.CodeUtil.i18n("File list");
...
% -----------------------------------------------------------------------
v_layout = addVerticalGridLayout(app_v_container);
folder_ui = bev1mus.AppUtil.Component.EditField(v_layout);
...
table_ui = bev1mus.AppUtil.Component.Table(v_layout);
...
if not(isfield(NameValuePair, "ColumnName"))
  if width(FileList) == 2
    % Default setting
    table_ui.MainTable.ColumnName = [bev1mus.CodeUtil.i18n("File path"), bev1mus.CodeUtil.i18n("Line number")];
  end

elseif isfield(NameValuePair, "ColumnName")
  if numel(NameValuePair.ColumnName) < 3
    id = errorID + "InvalidColumnName";
    msg = bev1mus.CodeUtil.i18n("ColumnName must have 3 or more elements.");
...
elseif isfield(NameValuePair, "ColumnWidth")
  if numel(NameValuePair.ColumnWidth) < 3
    ...
    msg = bev1mus.CodeUtil.i18n("ColumnWidth must have 3 or more elements.");
...
default_message = bev1mus.CodeUtil.i18n("Double-click to open the file.");

message_ui = bev1mus.AppUtil.Component.Label(v_layout);
...
% Callback functions
  ...
  function react_TableDoubleClicked(row_number)
    ...
    if not(isfile(file_fullpath))
      msg = bev1mus.CodeUtil.i18n("File not found:") + newline + replace(file_fullpath, ("/"|"\"), " > ");
