classdef AppWindow < handle
  ...
  properties
    ...
    MainVerticalContainer bev1mus.AppUtil.VerticalContainer

    HeaderUI bev1mus.AppUtil.Component.WindowHeader
  ...
  properties (Constant, Access=private)
    ...
    % The default icon file must exist in the "+bev1mus.AppUtil" namespace folder.
  ...
  methods
    ...
    function AppWindow = AppWindow(MainFigure, NameValuePair)
      ...
      if AppWindow.Reporting
        bev1mus.FileUtil.displayTimeAndFileLocation("Constructor")
      end  % if

      AppWindow.MainFigure = MainFigure;

      AppWindow.MainVerticalContainer = bev1mus.AppUtil.VerticalContainer(MainFigure);

      AppWindow.Icon = AppWindow.DefaultIcon;

      v_layout = addVerticalGridLayout(AppWindow.MainVerticalContainer);
      AppWindow.HeaderUI = bev1mus.AppUtil.Component.WindowHeader(v_layout);
    ...
    function set.Icon(AppWindow, PNGFilename)
      ...
      if PNGFilename == AppWindow.DefaultIcon
        try
          AppUtil_folder_fullpath = bev1mus.FileUtil.getFolderFullPath(fullfile("+bev1mus", "+AppUtil"));
        catch exception
        ...
        % Custom icon
        try
          icon_fullpath = bev1mus.FileUtil.getFileFullPath(PNGFilename);
        catch exception
          if AppWindow.MainFigure.Visible
            title_word = bev1mus.CodeUtil.i18n("Error");
