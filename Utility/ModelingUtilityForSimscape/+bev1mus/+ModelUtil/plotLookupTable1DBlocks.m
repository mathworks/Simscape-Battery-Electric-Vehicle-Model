...
if SubsystemPath == ""
  if isempty(gcs)
    ...
    msg = bev1mus.CodeUtil.i18n("Model must be specified.");
...
all_blocks = bev1mus.ModelUtil.findLookupTable1DBlocks(SubsystemPath, SearchDepth = NameValuePair.SearchDepth);
num_blocks = numel(all_blocks);
if num_blocks == 0

  id = errorID + "BlocksWereNotFound";
  msg = bev1mus.CodeUtil.i18n("No lookup table blocks were found.");
...
if not(isfield(NameValuePair, "Blocks"))
  ...
  for idx = 1 : numel(specified_blocks)
    ...
    if not(any(logical_index))
      ...
      msg = bev1mus.CodeUtil.i18n("Specified block was found: " + specified_blocks(idx));
...
for idx = 1 : num_blocks
  ...
  if bev1mus.ModelUtil.isSimulink1DLookupTableBlock(block_path)
    xdata = eval(get_param(block_path, "BreakpointsForDimension1"));
    xmaxvalues(idx) = xdata(end);
  elseif bev1mus.ModelUtil.isSimscapePSLookupTable1DBlock(block_path)
...
for idx = 1 : num_blocks
  ...
  if bev1mus.ModelUtil.isSimulink1DLookupTableBlock(block_fullpath)
    ...
    bev1mus.SignalUtil.plotSimulink1DLookupTableBlock(block_fullpath, ParentAxes=nexttile(vertical_tile), PlotXUpperBound=x_max, ...
  ...
  elseif bev1mus.ModelUtil.isSimscapePSLookupTable1DBlock(block_fullpath)
    unit_text = string(get_param(block_fullpath, "f_unit"));
    if unit_text ~= "1"
      title_text = title_text + " (" + unit_text + ")";
    end  % if
    bev1mus.SignalUtil.plotSimscapePSLookupTable1DBlock(block_fullpath, ParentAxes=nexttile(vertical_tile), PlotXUpperBound=x_max, ...
      DivisionType = NameValuePair.DivisionType, ...
      Divisions = NameValuePair.Divisions, ....
      InterpolationInterval = NameValuePair.InterpolationInterval, ...
      Title = title_text )
