...
if ModelName == ""
  ...
  msg = bev1mus.CodeUtil.i18n("Model name must be non-empty.");
...
result = bev1mus.FileUtil.getFileFullPath(ModelName, ReturnIfNotFound=true);
if result ~= ""
  if not(endsWith(result, ".mdl")) && not(endsWith(result, ".slx"))
    id = errorID + "InvalidFileName";
    msg = bev1mus.CodeUtil.i18n("Model file must ends with "".mdl"" or "".slx"".");
...
result = bev1mus.FileUtil.getFileFullPath(ModelName+".mdl", ReturnIfNotFound=true);
if result ~= ""
  FullPath = result;
...
result = bev1mus.FileUtil.getFileFullPath(ModelName+".slx", ReturnIfNotFound=true);
if result ~= ""
  FullPath = result;
...
msg = bev1mus.CodeUtil.i18n("Model file was not found.");
