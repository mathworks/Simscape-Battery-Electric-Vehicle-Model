...
%     "/path/to/file2"
%     ];
%   result = bev1mus.SearchUtil.replaceText( ...
%     file_paths, ...
%     DryRun = true, ...
%     TextPattern = "old",
%     IgnoreCase = true, ...
...
arguments (Input)
  ...
  NameValuePair.CommandName (1,1) string = "bev1mus.SearchUtil.replaceText"
...
if not(isfield(NameValuePair, "FilePaths"))
  ...
  msg = bev1mus.CodeUtil.i18n("FilePaths must be specified.");
...
if not(isfield(NameValuePair, "TextPattern"))
  ...
  msg = bev1mus.CodeUtil.i18n("TextPattern must be specified.");
...
if not(isfield(NameValuePair, "IgnoreCase"))
  ...
  msg = bev1mus.CodeUtil.i18n("IgnoreCase must be specified.");
...
if not(isfield(NameValuePair, "MatchWholeWord"))
  ...
  msg = bev1mus.CodeUtil.i18n("MatchWholeWord must be specified.");
...
if not(isfield(NameValuePair, "NewText"))
  ...
  msg = bev1mus.CodeUtil.i18n("NewText must be specified.");
...
if isempty(which(NameValuePair.CommandName))
  ...
  msg = bev1mus.CodeUtil.i18n("The command specified for CommandName must be on MATLAB path: ") + NameValuePair.CommandName;
...
if pat_str == ""
  ...
  msg = bev1mus.CodeUtil.i18n("TextPattern must be non-empty.");
...
if new_text == ""
  ...
  msg = bev1mus.CodeUtil.i18n("NewText must be non-empty.");
