...
if ModelName == ""
  ...
  msg = bevutil1.CodeUtil.i18n("Model name must be non-empty.");
...
result = bevutil1.FileUtil.getFileFullPath(ModelName, ReturnIfNotFound=true);
if result ~= ""
  if not(endsWith(result, ".mdl")) && not(endsWith(result, ".slx"))
    id = errorID + "InvalidFileName";
    msg = bevutil1.CodeUtil.i18n("Model file must ends with "".mdl"" or "".slx"".");
...
result = bevutil1.FileUtil.getFileFullPath(ModelName+".mdl", ReturnIfNotFound=true);
if result ~= ""
  FullPath = result;
...
result = bevutil1.FileUtil.getFileFullPath(ModelName+".slx", ReturnIfNotFound=true);
if result ~= ""
  FullPath = result;
...
msg = bevutil1.CodeUtil.i18n("Model file was not found.");
