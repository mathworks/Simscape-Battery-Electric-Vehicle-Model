...
%     "/path/to/file2"
%     ];
%   result = bevutil1.SearchUtil.replaceText( ...
%     file_paths, ...
%     DryRun = true, ...
%     TextPattern = "old",
%     IgnoreCase = true, ...
...
arguments (Input)
  ...
  NameValuePair.CommandName (1,1) string = "bevutil1.SearchUtil.replaceText"
...
if not(isfield(NameValuePair, "FilePaths"))
  ...
  msg = bevutil1.CodeUtil.i18n("FilePaths must be specified.");
...
if not(isfield(NameValuePair, "TextPattern"))
  ...
  msg = bevutil1.CodeUtil.i18n("TextPattern must be specified.");
...
if not(isfield(NameValuePair, "IgnoreCase"))
  ...
  msg = bevutil1.CodeUtil.i18n("IgnoreCase must be specified.");
...
if not(isfield(NameValuePair, "MatchWholeWord"))
  ...
  msg = bevutil1.CodeUtil.i18n("MatchWholeWord must be specified.");
...
if not(isfield(NameValuePair, "NewText"))
  ...
  msg = bevutil1.CodeUtil.i18n("NewText must be specified.");
...
if isempty(which(NameValuePair.CommandName))
  ...
  msg = bevutil1.CodeUtil.i18n("The command specified for CommandName must be on MATLAB path: ") + NameValuePair.CommandName;
...
if pat_str == ""
  ...
  msg = bevutil1.CodeUtil.i18n("TextPattern must be non-empty.");
...
if new_text == ""
  ...
  msg = bevutil1.CodeUtil.i18n("NewText must be non-empty.");
