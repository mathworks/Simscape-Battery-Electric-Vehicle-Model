classdef AppWindow < handle
  ...
  properties
    ...
    MainVerticalContainer bevutil1.AppUtil.VerticalContainer

    HeaderUI bevutil1.AppUtil.Component.WindowHeader
  ...
  properties (Constant, Access=private)
    ...
    % The default icon file must exist in the "+bevutil1.AppUtil" namespace folder.
  ...
  methods
    ...
    function AppWindow = AppWindow(MainFigure, NameValuePair)
      ...
      if AppWindow.Reporting
        bevutil1.FileUtil.displayTimeAndFileLocation("Constructor")
      end  % if

      AppWindow.MainFigure = MainFigure;

      AppWindow.MainVerticalContainer = bevutil1.AppUtil.VerticalContainer(MainFigure);

      AppWindow.Icon = AppWindow.DefaultIcon;

      v_layout = addVerticalGridLayout(AppWindow.MainVerticalContainer);
      AppWindow.HeaderUI = bevutil1.AppUtil.Component.WindowHeader(v_layout);
    ...
    function set.Icon(AppWindow, PNGFilename)
      ...
      if PNGFilename == AppWindow.DefaultIcon
        try
          AppUtil_folder_fullpath = bevutil1.FileUtil.getFolderFullPath(fullfile("+bevutil1", "+AppUtil"));
        catch exception
        ...
        % Custom icon
        try
          icon_fullpath = bevutil1.FileUtil.getFileFullPath(PNGFilename);
        catch exception
          if AppWindow.MainFigure.Visible
            title_word = bevutil1.CodeUtil.i18n("Error");
