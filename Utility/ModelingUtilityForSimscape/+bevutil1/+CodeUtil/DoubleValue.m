classdef DoubleValue < handle
  ...
  % For example,
  %{
        dval = bevutil1.CodeUtil.DoubleValue;
  ...
  methods
    ...
    function processValueText(doubleValueObject, value_text)
      ...
      if not(isnan(x))
        ...
        if not(isa(result, "double"))
          ...
          msg = bevutil1.CodeUtil.i18n("ValueText must evaluate to double.");
    ...
    function set.MainDoubleValue(doubleValueObject, x)
      ...
      doubleValueObject.current_double_value = x;
      doubleValueObject.current_value_text = bevutil1.CodeUtil.stringify(x);
    end  % function
