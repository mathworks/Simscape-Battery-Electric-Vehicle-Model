...
for idx = 1 : num_blocks
  ...
  if bevutil1.ModelUtil.isSimulink1DLookupTableBlock(block_fullpath)
    ...
    bevutil1.SignalUtil.plotSimulink1DLookupTableBlock( ...
      block_fullpath, ...
      UpdateModel = false, ... Do not update the model within a loop.
      ParentAxes = nexttile(vertical_tile), ...
      PlotXUpperBound = x_max, ...
  ...
  elseif bevutil1.ModelUtil.isSimscapePSLookupTable1DBlock(block_fullpath)
    ...
    bevutil1.SignalUtil.plotSimscapePSLookupTable1DBlock( ...
      block_fullpath, ...
      UpdateModel = false, ... Do not update the model within a loop.
      ParentAxes = nexttile(vertical_tile), ...
      PlotXUpperBound = x_max, ...
