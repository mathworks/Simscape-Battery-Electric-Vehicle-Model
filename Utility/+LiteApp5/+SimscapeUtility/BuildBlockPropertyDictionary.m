function [PropertyDictionary, ErrorMessage] = BuildBlockPropertyDictionary(BlockPath)
...
arguments (Output)
  ...
  ErrorMessage string {mustBeScalarOrEmpty}
end  % arguments

% Return this empty dictionary if error.
empty_dictionary = configureDictionary("string", "string");
PropertyDictionary = empty_dictionary;

if BlockPath == "" || not(contains(BlockPath, "/"))
  ErrorMessage = "Specify block path.";
  return
end
...
if block_type ~= "SimscapeBlock" && block_type ~= "SimscapeComponentBlock"
  ErrorMessage = "Specified block is not a Simscape block: " + BlockPath;
  return
end
...
if all(size(mask_params) == [0 0])
  ErrorMessage = "Selected Simscape block has no parameters.";
  return
end
