classdef LiteAppWindow < handle
  ...
  properties
    ...
    MainLayout LiteApp5.LiteAppLayout

    HeaderUI LiteApp5.Component.WindowHeader
  ...
  properties (Access=private)
    % The default icon file must exist in the "+LiteApp5" namespace folder.
  ...
  methods
    ...
    function AppWindow = LiteAppWindow(NameValuePair)
      ...
      if AppWindow.Reporting
        LiteApp5.Utility.ReportTimeAndFileLocation("Constructor")
      end  % if

      AppWindow.MainFigure = uifigure(Visible = "off");

      AppWindow.MainLayout = LiteApp5.LiteAppLayout(AppWindow.MainFigure);
      ...
      if AppWindow.useWindowHeader
        if AppWindow.Reporting
          LiteApp5.Utility.ReportTimeAndFileLocation("Using window header")
        end  % if
        AppWindow.HeaderUI = LiteApp5.Component.WindowHeader(NewArea(AppWindow.MainLayout));
    ...
    function Show(AppWindow)
      ...
      if AppWindow.Reporting
        LiteApp5.Utility.ReportTimeAndFileLocation("Visible has been set to ""on""")
      end  % if
      ...
      if AppWindow.useWindowHeader
        if AppWindow.Reporting
          LiteApp5.Utility.ReportTimeAndFileLocation("Updating window header")
        end  % if

        AppWindow.HeaderUI.AppName = AppWindow.MainFigure.Name;

        if AppWindow.Reporting
          LiteApp5.Utility.ReportTimeAndFileLocation("AppWindow.MainFigure.Name: " + AppWindow.MainFigure.Name)
        end  % if
    ...
    function set.Icon(AppWindow, PNGFilename)
      ...
      if PNGFilename == AppWindow.DefaultIcon
        try
          liteapp_folder_fullpath = LiteApp5.Utility.getFolderFullPath("+LiteApp5");
        catch exception
        ...
        % Custom icon
        try
          icon_fullpath = LiteApp5.Utility.getFileFullPath(PNGFilename);
        catch exception
          if AppWindow.MainFigure.Visible
            title_word = LiteApp5.Utility.i18n("Error");
    ...
    function set.AlwaysOnTop(AppWindow, on_or_off)
      ...
      if AppWindow.Show_AlwaysOnTop_CheckBox
        ...
        if AppWindow.Reporting
          LiteApp5.Utility.ReportTimeAndFileLocation("AppWindow.HeaderUI.AlwaysOnTop: " + on_or_off)
        end  % if
      else
        if AppWindow.Reporting
          LiteApp5.Utility.ReportTimeAndFileLocation("AppWindow.Show_AlwaysOnTop_CheckBox: " + AppWindow.Show_AlwaysOnTop_CheckBox)
          LiteApp5.Utility.ReportTimeAndFileLocation("AppWindow.HeaderUI.AlwaysOnTop: " + on_or_off)
        end  % if
      end  % if
    end  % function
