...
%[text] 
[num_files_to_be_saved, tbl] = ModelTool1.saveModels( DryRun = true ); %[output:98e53862]
disp(num_files_to_be_saved) %[output:08dacc67]
head(tbl) %[output:9e2de35e]
%%
[num_files_to_be_saved, tbl] = ModelTool1.saveModels( DryRun = true, ...
  DisplayInfo = false, ...
  Target = "FolderTree", ...
...
%%
[num_files_to_be_saved, tbl] = ModelTool1.saveModels( DryRun = true, ... %[output:group:47f2a59b] %[output:184b1e09]
