...
all_blocks = ModelTool1.findLookupTable1DBlocks(SubsystemPath, SearchDepth = NameValuePair.SearchDepth);
...
for idx = 1 : num_blocks
  ...
  if ModelTool1.isSimulink1DLookupTableBlock(block_path)
    xdata = eval(get_param(block_path, "BreakpointsForDimension1"));
    xmaxvalues(idx) = xdata(end);
  elseif ModelTool1.isSimscapePSLookupTable1DBlock(block_path)
...
for idx = 1 : num_blocks
  ...
  if ModelTool1.isSimulink1DLookupTableBlock(block_fullpath)
    ...
    SignalTool2.plotSimulink1DLookupTableBlock(block_fullpath, ParentAxes=nexttile(vertical_tile), PlotXUpperBound=x_max, ...
  ...
  elseif ModelTool1.isSimscapePSLookupTable1DBlock(block_fullpath)
    unit_text = string(get_param(block_fullpath, "f_unit"));
    if unit_text ~= "1"
      title_text = title_text + " (" + unit_text + ")";
    end  % if
    SignalTool2.plotSimscapePSLookupTable1DBlock(block_fullpath, ParentAxes=nexttile(vertical_tile), PlotXUpperBound=x_max, ...
      DivisionType = NameValuePair.DivisionType, ...
      Divisions = NameValuePair.Divisions, ....
      InterpolationInterval = NameValuePair.InterpolationInterval, ...
      Title = title_text )
