...
RotationalFriction1.plotRotationalFrictionTorque( ...
  DataSource="dataset")
%}
%{
% Example:
% Make a plot using a custom data set.
ds = RotationalFriction1.RotationalFrictionDataSet( ...
  Initialization=true);
% Modify ds here.
ds.PlotVelocityUnit = "rev/s";
RotationalFriction1.plotRotationalFrictionTorque( ...
...
arguments (Input)
  ...
  NameValuePair.DataSet RotationalFriction1.RotationalFrictionDataSet {mustBeScalarOrEmpty}
  ...
  NameValuePair.PlotVelocityUnit (1,1) simscape.Unit {simscape.mustBeCommensurateUnit(NameValuePair.PlotVelocityUnit, "rad/s")} = "rad/s"
...
if NameValuePair.DataSource == "direct"
  ...
  data_source = NameValuePair;

  breakaway_torque = data_source.BreakawayTorque;
  breakaway_velocity = data_source.BreakawayVelocity;
  coulomb_torque = data_source.CoulombTorque;
  viscous_coefficient = data_source.ViscousCoefficient;

  num_velocity_values = data_source.NumVelocityValues;
  plot_velocity_unit = data_source.PlotVelocityUnit;
  ...
  if isfield(NameValuePair, "DataSet")
    ...
    data_source = RotationalFriction1.RotationalFrictionDataSet( ...
      Initialization=true);
  end  % if
