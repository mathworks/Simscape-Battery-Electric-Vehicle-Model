...
%[text] - **Basic model** (`MotorDriveUnit_Basic_refsub`) is the simplest model with the fewest parameters among the four models. It uses [Motor & Drive block](https://www.mathworks.com/help/sdl/ref/motordrive.html) from Simscape Driveline. This model takes torque command and computes power conversion between electrical and mechanical powers using the **single efficiency measurement model** to compute copper loss coefficient $k\_c${"editStyle":"visual"}. Irons loss $P\_{\\textrm{iron}}${"editStyle":"visual"} and fixed loss $P\_{\\textrm{fixed}}${"editStyle":"visual"} are not modeled. This model does not simulate temperature dynamics. See the [description](file:AbstractMotorEfficiencyApp_Description_bev1mus.html) for more information about efficiency.
...
%[text] - [bev1mus\_AbstractMotorEfficiencyApp](matlab:bev1mus_AbstractMotorEfficiencyApp) \
