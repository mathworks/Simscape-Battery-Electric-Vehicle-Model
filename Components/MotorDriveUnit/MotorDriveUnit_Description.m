...
%[text] - $\\tau\_{\\textrm{meas}}${"editStyle":"visual"} ... torque at which efficiency is measured \
...
%[text] - **Basic model** (`MotorDriveUnit_Basic_refsub`) is the simplest model with the fewest parameters among the four models. It uses [Motor & Drive block](https://www.mathworks.com/help/sdl/ref/motordrive.html) from Simscape Driveline. This model takes torque command and computes power conversion between electrical and mechanical powers using the **single efficiency measurement model** to compute copper loss coefficient $k\_c${"editStyle":"visual"}. Irons loss $P\_{\\textrm{iron}}${"editStyle":"visual"} and fixed loss $P\_{\\textrm{fixed}}${"editStyle":"visual"} are not modeled. This model does not simulate temperature dynamics. See the [description](file:AbstractMotorEfficiencyApp_Description_bevutil1.html) for more information about efficiency.
...
%[text] - **System thermal model** (`MotorDriveUnit_refsub_System`) uses [Motor & Drive (System-Level) block](https://www.mathworks.com/help/sps/ref/motordrivesystemlevel.html) from Simscape Electrical to compute power conversion between electrical and mechanical powers. Thermal model is enabled too. Power conversion model is the same as the one in Basic and Basic thermal models above, i.e., the **single efficiency measurement model**, but irons loss $P\_{\\textrm{iron}}${"editStyle":"visual"} and fixed loss $P\_{\\textrm{fixed}}${"editStyle":"visual"} are also considered in this model. See the [description](file:AbstractMotorEfficiencyApp_Description_bevutil1.html) for more information about efficiency.
...
%[text] Use the abstract motor efficiency app to see the power conversion efficiency map of the motor. The link below works to open the app if you are viewing this document in MATLAB Web Browser or editing in MATLAB Editor.
%[text] - [bevutil1\_AbstractMotorEfficiencyApp](matlab:bevutil1_AbstractMotorEfficiencyApp) \
