classdef uptodateTest_Reducer < matlab.unittest.TestCase
  ...
  methods (Test)
    ...
    function harness_model_screenshot_is_uptodate(testcase)
      %%
      top_folder = fullfile(currentProject().RootFolder, "Components", "Reducer");
      verifyTrue(testcase, isfolder(top_folder))

      model_name = "HarnessModel_Reducer";
      image_filename = "screenshot-HarnessModel_Reducer.png";

      source_fullpath = fullfile(currentProject().RootFolder, "Components", "Reducer", model_name + ".mdl");
      verifyTrue(testcase, isfile(source_fullpath))

      destination_folder = fullfile(currentProject().RootFolder, "Components", "Reducer", "media");
