classdef unittest_Reducer_Basic_SimulationCases < matlab.unittest.TestCase
  ...
  properties
    ...
    ComponentID (1,1) string = "Reducer"
  ...
  methods (TestClassSetup)
    ...
    function setup_ModelID(testcase)
      ...
      % For example, extract "Basic" from "test_Reducer_Basic_SimulationCases".
      id = extractBetween(string(mfilename), "test_" + testcase.ComponentID + "_", "_SimulationCases");
  ...
  methods (Test)
    ...
    function PassingTest_2(testcase)
      % Run script, for example, Reducer_Basic_Constant.
      target_name = testcase.ComponentID + "_" + testcase.ModelID + "_Case1";
      target_fullpath = FileTool2.getFileFullPath(target_name);
      disp("Testing: " + target_fullpath)
      evalin("base", target_name)  % !test-target
    end  % function
