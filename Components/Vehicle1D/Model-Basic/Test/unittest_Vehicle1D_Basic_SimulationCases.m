classdef unittest_Vehicle1D_Basic_SimulationCases < matlab.unittest.TestCase
  ...
  methods (TestClassSetup)
    ...
    function setup_ModelID(testcase)
      target_folder = fullfile(extractBefore(string(pwd), ("/"|"\")+"Test"), "SimulationCases");
      disp("Target: " + target_folder)

      % Extract text from the target folder string.
      % For example, extract "Basic" from "Model-Basic/SimulationCases".
      testcase.ModelID = extractBetween(target_folder, "Model-", filesep+"SimulationCases");

      % Extract text from the class name.
      % For example, extract "Basic" from "unittest_Vehicle1D_Basic_SimulationCases".
