classdef uptodateTest_ElevationProfile < matlab.unittest.TestCase
  ...
  methods (Test)
    ...
    function description_html_is_uptodate(testcase)
      %%
      source_fullpath = bevutil1.FileUtil.getFileFullPath("ElevationProfile_Description.m");

      destination_fullpath = fullfile(currentProject().RootFolder, "Components", "ElevationProfile", "ElevationProfile_Description.html");
    ...
    function harness_model_screenshot_is_uptodate(testcase)
      %%
      top_folder = fullfile(currentProject().RootFolder, "Components", "ElevationProfile");
      verifyTrue(testcase, isfolder(top_folder))

      model_name = "HarnessModel_ElevationProfile";
      image_filename = "screenshot-HarnessModel_ElevationProfile.png";

      source_fullpath = fullfile(currentProject().RootFolder, "Components", "ElevationProfile", model_name + ".mdl");
      verifyTrue(testcase, isfile(source_fullpath))

      destination_folder = fullfile(currentProject().RootFolder, "Components", "ElevationProfile", "media");
