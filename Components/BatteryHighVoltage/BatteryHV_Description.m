...
%[text] - **Primitive** model ... is the simplest model and computes the voltage and current of the battery with no temperature dependence.
%[text] - **Basic** model ... is built with [Battery (System-Level) block](https://www.mathworks.com/help/sdl/ref/batterysystemlevel.html) from Simscape Driveline. This model has a simple equation-based terminal voltage model computed from the state of charge (SOC). This model also computes the battery temperature.
%[text] - **BasicThermal** model ... same as the Basic model except that battery temperature is computed.
%[text] - **SystemThermal** model ... is built with [Battery block](https://www.mathworks.com/help/sps/ref/battery.html) from Simscape Battery and Simscape Electrical. This model can simulate the terminal voltage more accurately than the Basic model but requires more parameters. Optionally, this model can also simulate charging dynamics, fade, and aging.
...
%[text] To validate the component, a harness model is used to run some simulation cases for each model. See scripts in **SimulationCases** folder in each model.
