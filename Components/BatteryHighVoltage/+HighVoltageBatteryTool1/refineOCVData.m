...
arguments (Input)
  ...
  NameValuePair.SOC (:,1) {mustBeBetween(NameValuePair.SOC, 0, 1, "closed"), bevutil1.CodeUtil.mustBeStrictAscend} = [0; 0.1; 0.15; 0.25; 0.75; 0.9; 1]
...
if height(soc) ~= height(ocv)
  ...
  msg = bevutil1.CodeUtil.i18n("SOC vector and OCV matrix must have the same number of rows.");
...
if width(ocv) ~= width(t)
  ...
  msg = bevutil1.CodeUtil.i18n("Temperature vector and OCV matrix must have the same number of columns.");
