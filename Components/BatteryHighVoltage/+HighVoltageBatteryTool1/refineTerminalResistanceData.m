...
arguments (Input)
  NameValuePair.Temperature (1,:) {bevutil1.CodeUtil.mustBeSimscapeValueStrictAscend} = simscape.Value([0, 25, 60], "degC")
  NameValuePair.SOC (:,1) {mustBeBetween(NameValuePair.SOC, 0, 1, "closed"), bevutil1.CodeUtil.mustBeStrictAscend} = [0; 0.05; 0.15; 0.4; 0.8; 1]
  NameValuePair.SOCInterval (1,1) {mustBeBetween(NameValuePair.SOCInterval, 0, 0.2, "open")} = 0.01
  NameValuePair.TerminalResistance {bevutil1.CodeUtil.mustBeSimscapeValuePositiveOrNan} = simscape.Value([0.47 0.17 0.13; 0.45 0.12 0.005; 0.218 0.072 0.04; 0.101 0.053 0.037; 0.086 0.044 0.03; 0.1 0.033 0.024], "Ohm")
end  % arguments
...
if height(soc) ~= height(r0)
  ...
  msg = bevutil1.CodeUtil.i18n("SOC vector and TerminalResistance matrix must have the same number of rows.");
...
if width(r0) ~= width(t)
  ...
  msg = bevutil1.CodeUtil.i18n("TerminalResistance matrix and Temperature vector must have the same number of columns.");
...
if ds > (soc(end) - soc(1))/2
  ...
  msg = bevutil1.CodeUtil.i18n("SOC interval is too large to insert data points.");
