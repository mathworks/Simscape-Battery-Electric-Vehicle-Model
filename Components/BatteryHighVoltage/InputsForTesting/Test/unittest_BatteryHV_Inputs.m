classdef unittest_BatteryHV_Inputs < matlab.unittest.TestCase
  ...
  methods (Test)
    ...
    function PassingTest_sim_2(~)
      model_name = "HarnessModel_BatteryHV_Inputs";
      load_system(model_name)
      ClickFcnText = get_param(model_name+"/Set inputs 1", "ClickFcn");  %!test-target
      evalin("base", ClickFcnText)
      sim(model_name);  % !test-target
    end  % function

    function PassingTest_sim_3(~)
      model_name = "HarnessModel_BatteryHV_Inputs";
      load_system(model_name)
      ClickFcnText = get_param(model_name+"/Set inputs 2", "ClickFcn");  %!test-target
      evalin("base", ClickFcnText)
      sim(model_name);  % !test-target
    end  % function

    function PassingTest_sim_4(~)
      model_name = "HarnessModel_BatteryHV_Inputs";
      load_system(model_name)
      ClickFcnText = get_param(model_name+"/Set inputs 3", "ClickFcn");  %!test-target
      evalin("base", ClickFcnText)
      sim(model_name);  % !test-target
    end  % function
