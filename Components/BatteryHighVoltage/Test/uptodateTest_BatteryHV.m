classdef uptodateTest_BatteryHV < matlab.unittest.TestCase
  ...
  methods (Test)
    ...
    function description_html_is_uptodate(testcase)
      %%
      source_fullpath = bevutil1.FileUtil.getFileFullPath("BatteryHV_Description.m");

      destination_fullpath = fullfile(currentProject().RootFolder, "Components", "BatteryHighVoltage", "BatteryHV_Description.html");
    ...
    function harness_model_screenshot_is_uptodate(testcase)
      %%
      top_folder = fullfile(currentProject().RootFolder, "Components", "BatteryHighVoltage");
      verifyTrue(testcase, isfolder(top_folder))

      model_name = "HarnessModel_BatteryHV";
      image_filename = "screenshot-HarnessModel_BatteryHV.png";

      source_fullpath = fullfile(currentProject().RootFolder, "Components", "BatteryHighVoltage", model_name + ".mdl");
      verifyTrue(testcase, isfile(source_fullpath))

      destination_folder = fullfile(currentProject().RootFolder, "Components", "BatteryHighVoltage", "media");
