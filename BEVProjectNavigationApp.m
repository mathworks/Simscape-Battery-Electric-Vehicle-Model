...
target_app = "bevutil1_Vehicle1DForceApp";
...
target_app = "bevutil1_AbstractMotorEfficiencyApp";
check_link(target_app);
...
target_hvbattery_visualization_script = "SampleScript_BatteryHV_refineOCVData";
...
target_hvbattery_parameter_build_script = "SampleScript_BatteryHV_refineTerminalResistanceData";
...
h_layout = addHorizontalGridLayout(h_container);
v_subcontainer = bevutil1.AppUtil.VerticalContainer(h_layout);

v_sublayout = addVerticalGridLayout(v_subcontainer);
link_ui = bevutil1.AppUtil.Component.Hyperlink(v_sublayout);
...
v_sublayout = addVerticalGridLayout(v_subcontainer);
doclink_ui = bevutil1.AppUtil.Component.Hyperlink(v_sublayout);
...
% -----------------------------------------------------------------------------
  ...
  function check_link(filename)
    ...
    try
      bevutil1.FileUtil.getFileFullPath(filename);
    catch exception
      if not(force_start)
