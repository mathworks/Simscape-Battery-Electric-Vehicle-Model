...
Open the [BEV system model](matlab:openFileInProject('BEV_system_model')). You can also use the shortcut button **BEV model** in the **Project Shortcuts** toolstrip.
...
Open the [Vehicle 1D harness model](matlab:openFileInProject('HarnessModel_Vehicle1D')). You can simulate and analyse basic vehicle performance with the Vehicle1D harness model.
...
Open the [High voltage battery harness model](matlab:openFileInProject('HarnessModel_BatteryHV')).
...
Open the [Motor drive unit harness model](matlab:openFileInProject('HarnessModel_MotorDriveUnit')).
